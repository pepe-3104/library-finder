...
export const useLibrarySearch = () => {
  ...
  const searchNearbyLibraries = useCallback(async (latitude, longitude, limit = 20) => {
    setLoading(true);
    setError(null);
    setLibraries([]);
    ...
    try {
      ...
      const apiUrl = `https://api.calil.jp/library?geocode=${latitude},${longitude}&limit=${limit}&format=json&callback=?`;
      ...
      if (response && response.length > 0) {
        ...
        console.log(`📚 ${formattedLibraries.length}件の図書館が見つかりました:`, formattedLibraries);
      } else {
        setLibraries([]);
...
const makeJsonpRequest = (url) => {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => {
      ...
      document.head.removeChild(script);
      delete window[callbackName];
      reject(new Error('図書館データの取得に失敗しました。ネットワーク接続を確認してください。'));
    };
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error('図書館検索がタイムアウトしました。時間をおいて再度お試しください。'));
    }, 15000);
