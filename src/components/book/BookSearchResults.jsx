...
const getBranchStatusList = (systems, libraries) => {
  ...
  sortedLibraries.forEach((library) => {
    ...
    if (
      systemInfo &&
      systemInfo.libkey &&
      Object.keys(systemInfo.libkey).length > 0
    ) {
      // 各分館を個別に表示
      Object.entries(systemInfo.libkey).forEach(([libId, libInfo]) => {
        ...
        if (shouldDisplay) {
          ...
          branchStatusList.push({
            ...
            libInfo,
            reserveUrl:
              systemInfo.reserveurl ||
              (isLibInfoObject ? libInfo.reserveurl : undefined),
    ...
    <div className="search-results">
      <div className="results-header">
        ...
        <p className="results-info">
          {searchType === "popular"
            ? `${searchQuery}: 総数${totalCount || 0}冊中 ${
                results.length
              }冊表示中`
            : `"${searchQuery}" の検索結果: 総数${totalCount || 0}冊中 ${
                results.length
              }冊表示中`}
          {pageInfo &&
            searchType !== "popular" &&
      ...
      <div className="results-list">
        {results.map((book, index) => (
          <BookResultItem
            ...
            rankNumber={
              searchType === "popular"
                ? ((pageInfo?.page || 1) - 1) * ITEMS_PER_PAGE + index + 1
                : null
            }
          />
        ))}
  ...
  return (
    <div
      className={`book-result-item ${
        book.isFutureRelease ? "future-release" : ""
      }`}
    >
      {/* ランキングバッジ（人気の本の場合のみ表示） */}
      {searchType === "popular" && rankNumber && (
        <div className="rank-badge">#{rankNumber}</div>
      )}
              ...
              <>
                ...
                {Object.keys(book.systems || {}).length > 0 && (
                  ...
                  >
                    <div
                      onClick={() =>
                        setIsLibraryInfoExpanded(!isLibraryInfoExpanded)
                      }
                      style={{
                        margin: "0 0 8px 0",
                        fontSize: "0.9rem",
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center" }}>
                        ...
                        />
                        蔵書情報
                        {getBranchStatusList(book.systems || {}, libraries)
                          .length > 0 && (
                          <span
                            style={{
                              marginLeft: "8px",
                        ...
                        {book.isLibraryDataLoading &&
                          book.librarySearchProgress && (
                            ...
                            >
                              ({book.librarySearchProgress.completed}/
                              {book.librarySearchProgress.total}館検索済み)
                            </span>
                          )}
                      </div>
                      <div style={{ display: "flex", alignItems: "center" }}>
                    ...
                    {isLibraryInfoExpanded &&
                      getBranchStatusList(book.systems || {}, libraries).map(
                        (branch, index) => {
                          ...
                          return (
                            <div
                              key={`${branch.systemid}-${branch.libId}-${index}`}
                                ...
                                {branch.status === "loading" ? (
                                  ...
                                  <div
                                    style={{
                                      ...
                                      alignItems: "center",
                                      backgroundColor:
                                        statusInfo.class === "available"
                                          ? "#d4edda"
                                          : statusInfo.class === "unavailable"
                                          ? "#f8d7da"
                                          : statusInfo.class === "reservable"
                                          ? "#fff3cd"
                                          : "#e9ecef",
                                      color:
                                        statusInfo.class === "available"
                      ...
                      >
                        ...
                        >
                          ...
                          {book.librarySearchProgress && (
                            ...
                            >
                              進捗: {book.librarySearchProgress.completed}/
                              {book.librarySearchProgress.total}館検索済み
                            </span>
                          )}
                        </Box>
      ...
      {book.isLibraryDataLoaded &&
        Object.keys(book.systems).length > 0 &&
        getTotalLibrariesCount(book.systems, libraries, userLocation) > 0 && (
          <div
            className="library-systems"
            style={{
              width: "100%",
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className="library-status-distance"
                          style={{
                            display: "flex",
                            flexDirection: "column",
                            alignItems: "flex-end",
                        ...
                        >
                          <span
                            className={`library-status ${statusInfo.class}`}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              backgroundColor:
